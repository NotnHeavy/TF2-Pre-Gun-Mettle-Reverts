...
//////////////////////////////////////////////////////////////////////////////
// PRE-GUN METTLE REVERTS; USES GPL-3, AS PER REQUEST OF SOURCEMOD          //
//////////////////////////////////////////////////////////////////////////////
...
#define PLUGIN_NAME "pre_gun_mettle_reverts"
#define CONFIG_PATH "addons/sourcemod/configs/" ... PLUGIN_NAME ... ".cfg"
...
ConVar pgm_reject_newitems;
ConVar pgm_use_old_deploy_speed;
...
Address MemoryPatch_DontMakeWrapAssassinBallExplode;
char MemoryPatch_DontMakeWrapAssassinBallExplode_OldValue[3];

Address MemoryPatch_DontBuildUberFasterInSetup;
Address MemoryPatch_DontBuildUberFasterInSetup_OldValue;
float MemoryPatch_DontBuildUberFasterInSetup_NewValue = 1.00;

Address MemoryPatch_DontDoubleRegenIfHealingHurtTarget;
Address MemoryPatch_DontDoubleRegenIfHealingHurtTarget_OldValue;
float MemoryPatch_DontDoubleRegenIfHealingHurtTarget_NewValue = 3.00;
...
Address CTFMinigun_m_flStartedWindUpAt; // *((_DWORD *)this + 554)
Address CTFMinigun_m_flStartedFiringAt;
...
RevertInfo weaponRevertInfo[0xFFFF] = { Revert_Full, ... };
bool classRevertToggled[view_as<int>(TFClass_Engineer) + 1] = { true, ... };
...
public void OnPluginStart()
{
    ...
    */
    ...
    MemoryPatch_DontMakeWrapAssassinBallExplode = GameConfGetAddress(config, "MemoryPatch_DontMakeWrapAssassinBallExplode");
    char MemoryPatch_DontMakeWrapAssassinBallExplode_NewValue[sizeof(MemoryPatch_DontMakeWrapAssassinBallExplode_OldValue) + 1];
    GameConfGetKeyValue(config, "MemoryPatch_DontMakeWrapAssassinBallExplode", MemoryPatch_DontMakeWrapAssassinBallExplode_NewValue, sizeof(MemoryPatch_DontMakeWrapAssassinBallExplode_NewValue));
    for (int i = 0; i < sizeof(MemoryPatch_DontMakeWrapAssassinBallExplode_OldValue); ++i)
    {
        Address index = view_as<Address>(i);
        MemoryPatch_DontMakeWrapAssassinBallExplode_OldValue[i] = LoadFromAddress(MemoryPatch_DontMakeWrapAssassinBallExplode + view_as<Address>(index), NumberType_Int8);
    }
    StoreToAddress(MemoryPatch_DontMakeWrapAssassinBallExplode, 0xC2, NumberType_Int8);
    StoreToAddress(MemoryPatch_DontMakeWrapAssassinBallExplode + view_as<Address>(1), GameConfGetOffset(config, "MemoryPatch_DontMakeWrapAssassinBallExplode_StackAdd"), NumberType_Int8);
    StoreToAddress(MemoryPatch_DontMakeWrapAssassinBallExplode + view_as<Address>(2), 0x00, NumberType_Int8);

    MemoryPatch_DontBuildUberFasterInSetup = GameConfGetAddress(config, "MemoryPatch_DontBuildUberFasterInSetup") + view_as<Address>(GameConfGetOffset(config, "MemoryPatch_DontBuildUberFasterInSetup"));
    MemoryPatch_DontBuildUberFasterInSetup_OldValue = LoadFromAddress(MemoryPatch_DontBuildUberFasterInSetup, NumberType_Int32);
    StoreToAddress(MemoryPatch_DontBuildUberFasterInSetup, AddressOf(MemoryPatch_DontBuildUberFasterInSetup_NewValue), NumberType_Int32);

    MemoryPatch_DontDoubleRegenIfHealingHurtTarget = GameConfGetAddress(config, "MemoryPatch_DontDoubleRegenIfHealingHurtTarget") + view_as<Address>(GameConfGetOffset(config, "MemoryPatch_DontDoubleRegenIfHealingHurtTarget"));
    MemoryPatch_DontDoubleRegenIfHealingHurtTarget_OldValue = LoadFromAddress(MemoryPatch_DontDoubleRegenIfHealingHurtTarget, NumberType_Int32);
    StoreToAddress(MemoryPatch_DontDoubleRegenIfHealingHurtTarget, AddressOf(MemoryPatch_DontDoubleRegenIfHealingHurtTarget_NewValue), NumberType_Int32);
    ...
    CTFMinigun_m_flStartedWindUpAt = view_as<Address>(FindSendPropInfo("CTFMinigun", "m_bCritShot") + 12);
    CTFMinigun_m_flStartedFiringAt = CTFMinigun_m_flStartedWindUpAt - view_as<Address>(4);
    ...
    pgm_reject_newitems = CreateConVar("pgm_reject_newitems", "1", "Disable the usage of post-Gun Mettle weapons. On by default.", FCVAR_PROTECTED);
    pgm_use_old_deploy_speed = CreateConVar("pgm_use_old_deploy_speed", "1", "Use the old deploy speed for every weapon (0.67s, instead of 0.5s). On by default.", FCVAR_PROTECTED);

    // ConCmds.
    RegAdminCmd("pgm_refresh_config", pgm_refresh_config, ADMFLAG_GENERIC, "Refresh configs/" ... PLUGIN_NAME ... ".cfg");
...
public void OnPluginEnd()
{
    ...
    */
    ...
    {
        Address index = view_as<Address>(i);
        StoreToAddress(MemoryPatch_DontMakeWrapAssassinBallExplode + index, MemoryPatch_DontMakeWrapAssassinBallExplode_OldValue[i], NumberType_Int8);
    }
    StoreToAddress(MemoryPatch_DontBuildUberFasterInSetup, MemoryPatch_DontBuildUberFasterInSetup_OldValue, NumberType_Int32);
    StoreToAddress(MemoryPatch_DontDoubleRegenIfHealingHurtTarget, MemoryPatch_DontDoubleRegenIfHealingHurtTarget_OldValue, NumberType_Int32);
}
...
// Read configs/pre_gun_mettle_reverts.txt to toggle any reverts.
void ReadRevertToggles()
{
    // Reset each toggle arrays.
    for (int i = 0; i < sizeof(weaponRevertInfo); ++i)
        weaponRevertInfo[i] = Revert_Full;
    for (int i = 0; i < sizeof(classRevertToggled); ++i)
        classRevertToggled[i] = true;

    // Open configs/pre_gun_mettle_reverts.txt into a KeyValues handle.
    KeyValues kv = new KeyValues("pgm");
    if (!kv.ImportFromFile(CONFIG_PATH))
    {
        PrintToServer("Could not find config path \"%s\", defaulting to toggling all reverts...", CONFIG_PATH);
        delete kv;
        return;
    }

    // Read from "itemdefs".
    if (!kv.JumpToKey("itemdefs"))
    {
        PrintToServer("Could not find key \"itemdefs\" in the config, defaulting to toggling all reverts...");
        delete kv;
        return;
    }

    // Read each item definition.
    if (kv.GotoFirstSubKey(false))
    {
        char section[32];
        do
        {
            // Verify the item definition.
            int itemdef;
            kv.GetSectionName(section, sizeof(section));
            if (StringToIntEx(section, itemdef) == 0 || itemdef != (itemdef & 0xFFFF))
                continue;

            // Read the toggle information.
            weaponRevertInfo[itemdef] = view_as<RevertInfo>(kv.GetNum(NULL_STRING, view_as<int>(Revert_Full)));
        }
        while (kv.GotoNextKey(false));
    }
    kv.GoBack();

    // Read each class toggle.
    classRevertToggled[TFClass_Scout] = !!kv.GetNum("scout", 1);
    classRevertToggled[TFClass_Soldier] = !!kv.GetNum("soldier", 1);
    classRevertToggled[TFClass_Pyro] = !!kv.GetNum("pyro", 1);
    classRevertToggled[TFClass_DemoMan] = !!kv.GetNum("demoman", 1);
    classRevertToggled[TFClass_Heavy] = !!kv.GetNum("heavy", 1);
    classRevertToggled[TFClass_Engineer] = !!kv.GetNum("engineer", 1);
    classRevertToggled[TFClass_Medic] = !!kv.GetNum("medic", 1);
    classRevertToggled[TFClass_Sniper] = !!kv.GetNum("sniper", 1);
    classRevertToggled[TFClass_Spy] = !!kv.GetNum("spy", 1);

    // Set Spy's movement speed.
    if (SpyClassData != Address_Null)
    {
        if (ClassRevertToggled(TFClass_Spy))
...
// Should this weapon be reverted?
bool RevertToggled(int itemdef)
{
    return weaponRevertInfo[itemdef] == Revert_Full;
}

// Should this class be reverted?
bool ClassRevertToggled(TFClassType class)
{
    return classRevertToggled[class];
}

// Modify the attributes of whatever weapon has just been assigned to the player.
public Action TF2Items_OnGiveNamedItem(int client, char[] class, int index, Handle& item)
{
    ...
    if (pgm_reject_newitems.BoolValue)
    {
    ...
    // Don't revert anything if this weapon is toggled off in the reverts config.
    if (weaponRevertInfo[index] == Revert_None)
        return Plugin_Continue;
 
    // Create a new item and change global attributes.
    newItem = TF2Items_CreateItem(OVERRIDE_ATTRIBUTES | PRESERVE_ATTRIBUTES);
    TF2Items_SetAttribute(newItem, 0, 773, (pgm_use_old_deploy_speed.BoolValue ? 1.34 : 1.00)); // This weapon deploys 1.34% slower. 0.5s * 1.34 = 0.67s, the same as pre-Tough Break switch speed.
    TF2Items_SetNumAttributes(newItem, 1);
    OriginalTF2ItemsIndex = -1;
    
    // Apply any custom attributes if specified.
    if (RevertToggled(index))
    {
        // Scout.
        {
            // Primary. 
            ...
            // Secondary.
            {
                ...
                else if (index == 812 || index == 833) // Flying Guillotine.
                {
                    // Apply new attributes.
        ...
        // Pyro. (:D)
        {
            // Primary.
            {
                if (StrEqual(class, "tf_weapon_flamethrower")) // Stats for all flamethrowers.
                {
                    // Apply new attributes.
                ...
                else if (index == 594) // Phlogistinator.
                {
                    // Apply new attributes.
        ...
        // Demoman.
        {
            // Primary.
            {
                ...
                else if (index == 996) // Loose Cannon. TODO: re-write knockback code?
                {
                    // Apply new attributes.
        ...
        // Sniper.
        {
            ...
            // Secondary.
            {
                ...
                else if (index == 751) // Cleaner's Carbine.
                {
                    // Remove old attributes.
...
int GetObjectOfType(int client, int iObjectType, int iObjectMode)
{
    ...
    for (int i = 0; i < iNumObjects; ++i)
    {
        ...
        if (SDKCall(SDKCall_CBaseObject_GetType, pObj) != iObjectType)
            continue;
        */
        if (GetEntProp(pObj, Prop_Send, "m_iObjectType") != iObjectType)
            continue;
...
public void ClientSpawn(Event event, const char[] name, bool dontBroadcast)
{
    ...
    if (DoesPlayerHaveItem(client, 173) && allPlayers[client].HadVitaSawEquipped && RevertToggled(173)) // Give up to 20% Uber back with the Vita-Saw.
...
public Action ClientDeath(Event event, const char[] name, bool dontBroadcast)
{
    ...
    {
        int index = GetWeaponIndex(weapon);
        if ((index == 140 && RevertToggled(140)) || (index == 1086 && RevertToggled(1086)) || (index == 30668 && RevertToggled(30668))) // Make the player's sentry shield disappear in only one second.
        {
...
public Action ClientBlastJumped(Event event, const char[] name, bool dontBroadcast)
{
    // Set up variables.
    int client = GetClientOfUserId(event.GetInt("userid"));
    if ((allPlayers[client].WeaponBlastJumpedWith == 237 || allPlayers[client].WeaponBlastJumpedWith == 265) && RevertToggled(allPlayers[client].WeaponBlastJumpedWith)) // Play the wind sound with the Rocket Jumper/Sticky Jumper
...
void RewardChargeOnChargeKill(int client) // This is called next frame to compensate for charge bash kills.
{
    ...
    {
        if (DoesPlayerHaveItem(client, chargeOnChargeKillWeapons[i][0]) && RevertToggled(chargeOnChargeKillWeapons[i][0]))
...
void SetEnergyRingHitbox(int entity)
{
    int launcher = GetEntPropEnt(entity, Prop_Send, "m_hLauncher");
    if (IsValidEntity(launcher) && RevertToggled(GetWeaponIndex(launcher)))
    {
...
public void OnEntityCreated(int entity, const char[] class)
{
    ...
    if (HasEntProp(entity, Prop_Send, "m_iItemDefinitionIndex")) // Any econ entity.
    {
        ...
        if (StrContains(class, "tf_weapon") == 0) // Hooks restricted to weapons only.
        {
            ...
            if (StrEqual(class, "tf_weapon_sniperrifle_decap") && RevertToggled(402))
    ...
    else if (StrEqual(class, "obj_sentrygun") || StrEqual(class, "obj_dispenser") || StrEqual(class, "obj_teleporter") && ClassRevertToggled(TFClass_Engineer)) // Engineer's buildings.
    {
    ...
    else if (StrEqual(class, "tf_projectile_healing_bolt") && RevertToggled(997))
...
public void OnGameFrame()
{
    ...
    // Disable the detour for getting the TFPlayerClassData_t object for Spy,
    // if we already have it.
    if (SpyClassData != Address_Null && DHooks_GetPlayerClassData != null)
    {
        DHookDisableDetour(DHooks_GetPlayerClassData, true, GetTFClassData);
        DHooks_GetPlayerClassData = null;
    }
    ...
    for (int i = 1; i <= MaxClients; ++i)
    {
        if (IsClientInGame(i))
        {
            ...
            // Shortstop push prevention and ammo management. The push prevention doesn't really account for after finishing reload. The DHook I have doesn't account for client prediction either.
            doesHaveWeapon = DoesPlayerHaveItem(i, 220);
            if (doesHaveWeapon && secondaryWeapon != -1 && GetEntProp(secondaryWeapon, Prop_Send, "m_iPrimaryAmmoType") == SCOUT_PISTOL_AMMO_TYPE && RevertToggled(220))
            {
            ...
            if (doesHaveWeapon && doesHaveWeapon == activeWeapon && GetEntProp(i, Prop_Data, "m_nWaterLevel") < WL_Waist && GetEntityMoveType(i) == MOVETYPE_WALK && !TF2_IsPlayerInCondition(i, TFCond_CritHype) && RevertToggled(448))
            {
                float speed[3];
            ...
            if (doesHaveWeapon && RevertToggled(GetWeaponIndex(doesHaveWeapon)))
            {
            ...
            if (doesHaveWeapon && RevertToggled(44))
            ...
            if (doesHaveWeapon && RevertToggled(648))
            ...
            if (doesHaveWeapon && RevertToggled(1101))
            {
            ...
            if (activeWeapon != -1 && GetWeaponIndex(activeWeapon) == 357 && RevertToggled(357) && GetEntProp(i, Prop_Send, "m_iKillCountSinceLastDeploy") == 0 && GetGameTime() >= GetEntPropFloat(i, Prop_Send, "m_flFirstPrimaryAttack"))
            ...
            if (doesHaveWeapon && RevertToggled(528))
                SetEntPropFloat(doesHaveWeapon, Prop_Send, "m_flNextSecondaryAttack", GetGameTime() + 1.00);
            ...
            if (doesHaveWeapon && RevertToggled(998))
            {
            ...
            if (doesHaveWeapon && IsPlayerAlive(i) && RevertToggled(GetWeaponIndex(doesHaveWeapon)))
            {
            ...
            if (doesHaveWeapon && allPlayers[i].UsingVaccinatorUber && RevertToggled(998))
                SetEntData(doesHaveWeapon, view_as<int>(CWeaponMedigun_m_bReloadDown), true, 1);

            // Bazaar Bargain head counter.
            int decapitations = GetEntProp(i, Prop_Send, "m_iDecapitations");
            if (((allPlayers[i].BazaarBargainShot == BazaarBargain_Lose && decapitations != 0) || allPlayers[i].BazaarBargainShot != BazaarBargain_Idle) && RevertToggled(402))
            {
            ...
            // Revert the spread/damage scaling of miniguns by force-setting the value of m_flStartedWindUpAt.
            if (activeWeapon != -1 && StrEqual(allEntities[activeWeapon].Class, "tf_weapon_minigun") && RevertToggled(GetWeaponIndex(activeWeapon)))
            {
                float startFiring = GetEntDataFloat(activeWeapon, view_as<int>(CTFMinigun_m_flStartedFiringAt));
                SetEntDataFloat(activeWeapon, view_as<int>(CTFMinigun_m_flStartedWindUpAt), startFiring);
            }
        }
    }
    ...
    if (frame % TICK_RATE == 0) // Every second.
    {
        for (int i = 1; i <= MaxClients; ++i)
        {
            if (IsClientInGame(i))
            {
                ...
                if (DoesPlayerHaveItem(i, 354) && health < maxHealth && RevertToggled(354)) // Concheror passive heal.
                {
                ...
                else if (DoesPlayerHaveItem(i, 642) && health < maxHealth && RevertToggled(642)) // Cozy Camper passive heal.
                {
                    // Show that attacker got healed.
                    Handle event = CreateEvent("player_healonhit", true);
...
public Action OnPlayerRunCmd(int client, int& buttons, int& impulse, float vel[3], float angles[3], int& weapon, int& subtype, int& cmdnum, int& tickcount, int& seed, int mouse[2])
{
    ...
    if (doesHaveWeapon && GetEntProp(doesHaveWeapon, Prop_Send, "m_iClip1") == 4 && RevertToggled(1153))
    {
    ...
    if (IsValidEntity(doesHaveWeapon) && GetWeaponIndex(doesHaveWeapon) == 528 && RevertToggled(528) && !GetEntProp(client, Prop_Send, "m_bHasPasstimeBall") && buttons & IN_ATTACK2)
        SDKCall(SDKCall_CTFPlayer_TryToPickupBuilding, client);

    // Prevent using alt-fire to activate the Amputator taunt.
    if (IsValidEntity(doesHaveWeapon) && GetWeaponIndex(doesHaveWeapon) == 304 && RevertToggled(304) && buttons & IN_ATTACK2)
    {
        buttons ^= IN_ATTACK2;
        return Plugin_Changed;
    }
    ...
    if (IsValidEntity(doesHaveWeapon))
    {
        ...
        if (((index == 56 && RevertToggled(56)) || (index == 1005 && RevertToggled(1005)) || (index == 1092 && RevertToggled(1092))) && GetEntityFlags(client) & FL_ONGROUND == 0 && allPlayers[client].WasInAttack)
        {
            buttons |= IN_ATTACK;
            return Plugin_Changed;
        }
...
Action EntitySpawn(int entity)
{
    ...
    else if (IsValidEntity(entity) && HasEntProp(entity, Prop_Send, "m_iWorldModelIndex") && HasEntProp(entity, Prop_Send, "m_iItemDefinitionIndex"))
    {
        ...
        RequestFrame(SetEnergyRingHitbox, entity);

    if (StrEqual(allEntities[entity].Class, "tf_weapon_handgun_scout_primary") && RevertToggled(220)) // Set the ammo type of the Shortstop, so that it uses the pistol's ammo reserve.
...
Action EntityTouch(int entity, int other)
{
    if (StrContains(allEntities[entity].Class, "tf_projectile_") == 0)
    {
        if (StrEqual(allEntities[entity].Class, "tf_projectile_energy_ring"))
        {
            ...
            int launcher = GetEntPropEnt(entity, Prop_Send, "m_hLauncher");
            if (other > 0 && other <= MaxClients && TF2_GetClientTeam(other) == TF2_GetClientTeam(GetEntPropEnt(entity, Prop_Send, "m_hOwnerEntity")) && RevertToggled(GetWeaponIndex(launcher)))
            {
                isTeammate = true;
            ...
            if (GetWeaponIndex(GetEntPropEnt(entity, Prop_Send, "m_hLauncher")) == 588 && RevertToggled(588) && (isTeammate || HasEntProp(other, Prop_Send, "m_hBuilder")))
                return Plugin_Handled;
        }
        ...
        if (other > 0 && other <= MaxClients)
        {
            ...
            if (StrEqual(allEntities[entity].Class, "tf_projectile_ball_ornament") && doesHaveWeapon && GetEntPropFloat(doesHaveWeapon, Prop_Send, "m_flEffectBarRegenTime") > GetGameTime() && RevertToggled(648))
            {
                RemoveEntity(entity);
...
Action BuildingDamaged(int victim, int& attacker, int& inflictor, float& damage, int& damagetype, int& weapon, float damageforce[3], float damageposition[3], int damagecustom)
{
    ...
    if (IsValidEntity(weapon))
    {
        ...
        int wrangler = DoesPlayerHaveItemByClass(victim, "tf_weapon_laser_pointer");
        if (wrangler && StrEqual(allEntities[victim].Class, "m_nShieldLevel") && RevertToggled(GetWeaponIndex(wrangler))) // Set sentry damage resistances from miniguns and the sapper's owner.
        {
            bool changed = false;
...
Action ClientDamaged(int victim, int& attacker, int& inflictor, float& damage, int& damagetype, int& weapon, float damageforce[3], float damageposition[3], int damagecustom)
{
    ...
    if (damagecustom == TF_CUSTOM_BASEBALL && GetWeaponIndex(inflictor) == 44 && RevertToggled(44)) // Sandman stun. The majority of this is sourced from the TF2 source code leak.
    {
        damage = 15.00; // Force the damage to always be 15.
    ...
    else if (damagecustom == TF_CUSTOM_CANNONBALL_PUSH && RevertToggled(996)) // Loose Cannon ball.
    {
        damage = 60.00;
        returnValue = Plugin_Changed;
    }
    else if ((damagecustom == TF_CUSTOM_CLEAVER || damagecustom == TF_CUSTOM_CLEAVER_CRIT) && allPlayers[victim].TicksSinceProjectileEncounter == GetGameTickCount() && GetGameTime() - allEntities[allPlayers[victim].MostRecentProjectileEncounter].SpawnTimestamp >= 1 && (RevertToggled(812) || RevertToggled(833))) // Flying Guillotine mini-crit.
        TF2_AddCondition(victim, TFCond_MarkedForDeathSilent, TICK_RATE_PRECISION);
    else if ((inflictorIndex == 228 || inflictorIndex == 1085) && RevertToggled(inflictorIndex) && attacker != victim && TF2_GetClientTeam(attacker) != TF2_GetClientTeam(victim)) // Black Box hit.
    {
        // Show that attacker got healed.
        Handle event = CreateEvent("player_healonhit", true);
    ...
    if (IsValidEntity(weapon) && RevertToggled(index)) 
    {
        ...
        if (index == 127 && TF2_IsPlayerInCondition(victim, TFCond_BlastJumping)) // Direct Hit mini-crit on players thrown into air via explosion.
            TF2_AddCondition(victim, TFCond_MarkedForDeathSilent, TICK_RATE_PRECISION);
        if (allPlayers[victim].TicksSinceHeadshot == GetGameTickCount() && (index == 61 || index == 1006)) // Ambassador headshot.
        {
...
MRESReturn OnTakeDamageAlive(int entity, DHookReturn returnValue, DHookParam parameters)
{
    ...
    for (int i = 1; i <= MaxClients; ++i)
    {
        if (allPlayers[victim].VaccinatorHealers[i])
        {
            ...
            if (attacker != victim && damagetype & resistanceMapping[GetResistType(vaccinator)] && RevertToggled(998)) // Check that the damage type matches the Medic's current resistance.
            {
...
Action ClientDamagedAlive(int victim, int &attacker, int &inflictor, float &damage, int &damagetype, int &weapon, float damageForce[3], float damagePosition[3], int damagecustom)
{
    ...
    if (TF2_IsPlayerInCondition(victim, TFCond_RestrictToMelee) && TF2_IsPlayerInCondition(victim, TFCond_CritCola) && GetGameTickCount() - allPlayers[victim].TicksSinceConsumingSandvich < TICK_RATE * LUNCHBOX_ADDS_MINICRITS_DURATION && RevertToggled(311)) // 25% damage vulnerability when under the effects of the Buffalo Steak Sandvich.
    {
        damage = damage / 1.20 * 1.25;
        returnValue = Plugin_Changed;
    }
    
    if (TF2_IsPlayerInCondition(victim, TFCond_CritCola) && TF2_GetPlayerClass(victim) == TFClass_Scout && RevertToggled(163)) // 10% damage vulnerability while using Crit-a-Cola.
    {
        damage *= 1.10;
        returnValue = Plugin_Changed;
    }

    if (DoesPlayerHaveItem(victim, 232) && damagetype & (DMG_IGNITE | DMG_BURN) && RevertToggled(232)) // 20% fire damage vulnerability with the Bushwacka.
    {
        damage *= 1.20;
        returnValue = Plugin_Changed;
    }

    if (DoesPlayerHaveItem(victim, 773) && RevertToggled(773)) // The player has the Pretty Boy's Pocket Pistol equipped.
    {
    ...
    {
        int index = GetWeaponIndex(weapon);
        if (index == 230 && RevertToggled(230) && GetGameTime() - allPlayers[attacker].TimeSinceScoping >= 1.0 && TF2_IsPlayerInCondition(attacker, TFCond_Slowed)) // Sydney Sleeper Jarateing.
...
{
    int victimActiveWeapon = GetEntPropEnt(victim, Prop_Send, "m_hActiveWeapon");
    if (IsValidEntity(victimActiveWeapon) && GetWeaponIndex(victimActiveWeapon) == 649 && damagetype & (DMG_IGNITE | DMG_BURN) && RevertToggled(649)) // Spy-cicle fire immunity.
    {
        // Add immunity.
    ...
    if (DoesPlayerHaveItem(victim, 1099) && TF2_IsPlayerInCondition(victim, TFCond_Charging) && RevertToggled(1099)) // Charge loss when taking damage with the Tide Turner.
    {
    ...
    if (allPlayers[victim].TicksSinceFeignReady == GetGameTickCount() && RevertToggled(59)) // Set the cloak meter to 100 when feigning.
...
void AfterClientSwitchedWeapons(int client, int weapon)
{
    ...
    {
        int index = GetWeaponIndex(weapon);
        if (index == 998 && allPlayers[client].UsingVaccinatorUber && RevertToggled(998)) // Give back resistances to the Medic if using the Vaccinator Uber.
        {
...
Action ClientGetMaxHealth(int client, int& maxhealth)
{
    ...
    {
        if (DoesPlayerHaveItem(client, weaponHealthModifiers[weapon][0]) && RevertToggled(weaponHealthModifiers[weapon][0]))
...
MRESReturn CAmmoPack_OnAmmoTouch(int entity, DHookReturn returnValue, DHookParam parameters)
{
    ...
    if (DoesPlayerHaveItem(client, 404) && RevertToggled(404)) // Health pickup with the Persian Persuader.
    {
...
MRESReturn WeaponEquipped(int entity, DHookParam parameters)
{
    ...
    if (client > 0 && client <= MaxClients && TF2_GetPlayerClass(client) == TFClass_Engineer && entity == GetPlayerWeaponSlot(client, TFWeaponSlot_Melee) && ClassRevertToggled(TFClass_Engineer))
...
{
    if (GetEntProp(entity, Prop_Send, "m_bMiniBuilding") && RevertToggled(142)) // Mini sentries always start off at max health.
...
MRESReturn PostConstructBuilding(int entity, DHookReturn returnValue, DHookParam parameters)
{
    ...
    if (GetEntProp(entity, Prop_Send, "m_bMiniBuilding") && RevertToggled(142)) // Prevent mini sentries from gaining health while being built. Usually only an issue if the building has been damaged during construction time.
    {
...
{
    if (GetEntProp(entity, Prop_Send, "m_bMiniBuilding") && RevertToggled(142)) // Do not allow reparations on mini sentries.
    {
        returnValue.Value = false;
        return MRES_Supercede;
    }
    if (RevertToggled(997))
    {
...
{
    int client = GetEntPropEnt(entity, Prop_Send, "m_hOwnerEntity");
    int weapon = DoesPlayerHaveItemByClass(entity, "tf_weapon_crossbow");
    if (client > 0 && weapon && RevertToggled(GetWeaponIndex(weapon))) // Do not grant Uber with the crossbow.
...
{
    if (parameters.Get(1) == OBJ_TELEPORTER && ClassRevertToggled(TFClass_Engineer)) // Revert the teleporter cost to 125. This won't affect the client however...
    {
...
MRESReturn CalculateMaxSpeed(int entity, DHookReturn returnValue)
{
    ...
    int index = GetWeaponIndex(activeWeapon);
    if (TF2_IsPlayerInCondition(entity, TFCond_CritCola) && TF2_GetPlayerClass(entity) == TFClass_Scout && RevertToggled(163)) // Crit-a-Cola speed boost.
    {
        returnValue.Value = view_as<float>(returnValue.Value) * 1.25;
        return MRES_Override;
    }
    else if (GetEntProp(entity, Prop_Send, "m_bCarryingObject") && !IsMannVsMachineMode() && !TF2_IsPlayerInCondition(entity, TFCond_HalloweenBombHead) && ClassRevertToggled(TFClass_Engineer)) // If an Engineer is carrying a building, slow him down by 25% instead of 10%.
    {
        returnValue.Value = view_as<float>(returnValue.Value) / 0.90 * 0.75;
        return MRES_Override;
    }
    else if (IsValidEntity(activeWeapon) && index != 411 && RevertToggled(index) && StrEqual(allEntities[activeWeapon].Class, "tf_weapon_medigun") && GetEntPropEnt(activeWeapon, Prop_Send, "m_hHealingTarget") != -1) // The player is a Medic who is not using the Quick-Fix and is healing a target. In that case, do not mirror their target's speed.
    {
...
{
    if (!ClassRevertToggled(TFClass_Scout))
        return MRES_Ignored;
...
{
    int activeWeapon = GetEntPropEnt(entity, Prop_Send, "m_hActiveWeapon");
    if (IsValidEntity(activeWeapon) && GetWeaponIndex(activeWeapon) == 594 && RevertToggled(594) && parameters.Get(1) == TAUNT_BASE_WEAPON && GetEntPropFloat(entity, Prop_Send, "m_flRageMeter") == 100) // Is player using mmmph with the Phlog?
    {
...
{
    if (DoesPlayerHaveItem(entity, 642) && TF2_IsPlayerInCondition(entity, TFCond_Zoomed) && RevertToggled(642)) // Cozy Camper flinching.
    {
        returnValue.Value = false;
        return MRES_Supercede;
    }
...
MRESReturn AddCondition(Address thisPointer, DHookParam parameters)
{
    ...
    if (condition == TFCond_MarkedForDeathSilent && TF2_IsPlayerInCondition(client, TFCond_CritCola) && allPlayers[client].TicksSinceAttack == GetGameTickCount() && RevertToggled(163)) // Do not mark the player for death when using Crit-a-Cola.
        return MRES_Supercede;
    else if ((condition == TFCond_UberchargedCanteen || condition == TFCond_MegaHeal) && allPlayers[client].TicksSinceMmmphUsage == GetGameTickCount() && RevertToggled(594)) // Phlog invulnerability/knockback prevention.
        return MRES_Supercede;
    else if (condition == TFCond_SpeedBuffAlly && RevertToggled(447))
    {
    ...
    else if ((condition == TFCond_RestrictToMelee || condition == TFCond_CritCola) && allPlayers[client].TicksSinceConsumingSandvich == GetGameTickCount() && RevertToggled(311)) // Shorten the Buffalo Steak Sandvich effects duration to 15s.
    {
        parameters.Set(2, LUNCHBOX_ADDS_MINICRITS_DURATION);
        return MRES_ChangedHandled;
    }
    else if ((condition == TFCond_UberBulletResist || condition == TFCond_UberBlastResist || condition == TFCond_UberFireResist) && RevertToggled(998)) // Vaccinator Uber should only last 2s.
    {
        parameters.Set(2, 2.0);
        return MRES_ChangedHandled;
    }
    else if (condition == TFCond_Cloaked && DoesPlayerHaveItem(client, 59) && RevertToggled(59)) // Simplify feign checks with the Dead Ringer.
    {
    ...
    else if (condition == TFCond_CloakFlicker && allPlayers[client].FeigningDeath && GetFeignBuffsEnd(client) >= GetGameTickCount() && RevertToggled(59)) // Prevent bump shimmers while feigning with the Dead Ringer.
        return MRES_Supercede;
    else if (condition == TFCond_AfterburnImmune && allPlayers[client].TicksSinceFeignReady == GetGameTickCount() && RevertToggled(59)) // Do not provide afterburn immunuity.
        return MRES_Supercede;
    else if (condition == TFCond_Slowed && GetPlayerWeaponSlot(client, TFWeaponSlot_Primary) == GetEntPropEnt(client, Prop_Send, "m_hActiveWeapon") && RevertToggled(230)) // Modify checks for the Sydney Sleeper.
    ...
    {
        int weapon = DoesPlayerHaveItems(client, { 46, 1145 }, 2);
        if (weapon && RevertToggled(GetWeaponIndex(weapon)))
            return MRES_Supercede;
    }
    return MRES_Ignored;
}
...
MRESReturn RemoveCondition(Address thisPointer, DHookParam parameters)
{
    ...
    {
        int weapon = DoesPlayerHaveItemByClass(client, "tf_wearable_demoshield");
        if (weapon && RevertToggled(GetWeaponIndex(weapon)))
        {
    ...
    else if (condition == TFCond_Cloaked && allPlayers[client].FeigningDeath && RevertToggled(59)) // End feign death with the Dead Ringer.
    {
    ...
    else if (condition == TFCond_Parachute && RevertToggled(1101)) // Used to check for parachute redeployment.
...
MRESReturn CalculateChargeCrit(Address thisPointer, DHookParam parameters)
{
    int client = GetEntityFromAddress(Dereference(thisPointer + CTFPlayerShared_m_pOuter));
    int weapon = DoesPlayerHaveItemByClass(client, "tf_wearable_demoshield");
    if (GetGameTime() - allPlayers[client].TimeSinceShieldBash < 0.5 && allPlayers[client].ChargeBashHitPlayer && weapon && RevertToggled(GetWeaponIndex(weapon)))
    {
...
{
    int client = GetEntityFromAddress(Dereference(thisPointer + CTFPlayerShared_m_pOuter));
    if (DoesPlayerHaveItem(client, 59) && RevertToggled(59)) // Only gain up to 35% cloak with the Dead Ringer.
    {
...
{
    int client = GetEntityFromAddress(Dereference(thisPointer + CTFPlayerShared_m_pOuter));
    if (!IsValidEntity(client) || !RevertToggled(411))
        return MRES_Ignored;
...
{
    int client = GetEntityFromAddress(Dereference(thisPointer + CTFPlayerShared_m_pOuter));
    if (TF2_GetPlayerClass(client) == TFClass_Pyro && DoesPlayerHaveItem(client, 594) && RevertToggled(594))
    {
...
{
    if (GetWeaponIndex(entity) == 447 && RevertToggled(447)) // Disciplinary Action speed boost. (This code only cares about the victim, the player using the Disciplinary Action already has their speed boost duration set to 3.6s.)
    {
...
{
    if (GetWeaponIndex(entity) == 57 && RevertToggled(57)) // Delete the Razorback entirely. This is basically how the old Razorback functioned.
        RemoveEntity(entity);
    return MRES_Ignored;
}
...
MRESReturn OnShieldBash(int entity)
{
    ...
    */
    ...
    if (!RevertToggled(GetWeaponIndex(entity)))
        return MRES_Ignored;
...
{
    int index = GetWeaponIndex(entity);
    if (index == 311 && RevertToggled(331)) // Buffalo Steak Sandvich.
...
MRESReturn BuildingConstructionHit(int entity, DHookParam parameters)
{
    ...
    if (!GetEntProp(entity, Prop_Send, "m_bMiniBuilding") && RevertToggled(142)) // Do not allow mini sentries to be construction boosted.
    {
        int client = parameters.Get(1);
...
MRESReturn GetBuildingConstructionMultiplier(int entity, DHookReturn returnValue)
{
    ...
    if (!ClassRevertToggled(TFClass_Engineer))
...
{
    if (GetEntProp(entity, Prop_Send, "m_bMiniBuilding") && RevertToggled(142)) // Allow metal to be picked up from mini sentry gibs.
    {
        parameters.Set(2, 7);
        return MRES_ChangedHandled;
    }
...
MRESReturn ApplyDamageRules(Address thisPointer, DHookReturn returnValue, DHookParam parameters)
{
    ...
    if (victim > 0 && victim <= MaxClients)
    {
        ...
        for (int i = 1; i <= MaxClients; ++i)
        {
            if (allPlayers[victim].VaccinatorHealers[i] && (bitsDamageType & resistanceMapping[GetResistType(DoesPlayerHaveItem(i, 998))]) && RevertToggled(998))
            {
...
{
    int victim = parameters.Get(2);
    if (victim > 0 && victim <= MaxClients && allPlayers[victim].TicksSinceApplyingDamageRules == GetGameTickCount() && RevertToggled(998))
    {
        Address info = parameters.Get(1);
...
Action pgm_refresh_config(int client, int args)
{
    ReadRevertToggles();
    ReplyToCommand(client, "[Pre-Gun Mettle Reverts]: Refreshed the reverts config.");
    return Plugin_Continue;
}
